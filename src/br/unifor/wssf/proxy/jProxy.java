 ...
 * -----------------------------------------------------
 * Altera��es:
 * 1) Cria��o da classe InvocationThread para ser utilizada pelo o framework WSSF.
 * 2) Delega��o do processo de invoca��o � uma ServerSelectionPolicy.
 * 3) Classe ProxyThread implementa a interface WSSFProxy.
 */
...
import br.unifor.wssf.core.policy.factory.ServerSelectionPolicyFactory;
...
//		debugOut = out;
	...
	public void run() {
		try {
			...
			// /////////////////////////////////////////////////////////////
			// Usando o framework WSSF
			boolean existeReplica = true; // TODO: Inserir c�digo de verifica��o
											// de exist�ncia de r�plicas.
...
//				}
				...
				if (response !=null) responseLength = Array.getLength(response);
				// /////////////////////////////////////////////////////////////
			} else {
				...
				if (pos > 0) {
					try {
						hostPort = Integer
								.parseInt(hostName.substring(pos + 1));
					} catch (Exception e) {
					}
				...
				if (server != null) {
					...
					BufferedInputStream serverIn = new BufferedInputStream(
					...
					// requires us to return the data in the response, just
					// stream
					...
					if (debugLevel > 1) {
						...
						responseLength = streamHTTPData(serverIn, clientOut,
								true);
					}
			...
			if (debugLevel > 1 || existeReplica) {
				// System.out.println("send the response back to the client, if we haven't already");
			...
			if (data != null) {
				...
				if ((data.toLowerCase().startsWith("http")) && (pos >= 0)
					...
					try {
						...
						if (debugLevel > 0)
							Log.e("proxy", "Error parsing response code " + rcString, e);
...
//						debugOut.println(errMsg);
			...
			if (debugLevel > 0)
				Log.e("proxy", "Error getting HTTP data: " + e.getMessage(), e);
//				debugOut.println("Error getting HTTP data: " + e);
	...
	private String readLine(InputStream in) {
		...
		try {
			...
			if (debugLevel > 0)
				Log.e("proxy", "Error getting header: " + e.getMessage(), e);
